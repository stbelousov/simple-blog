...
from flask import Flask, request, render_template, flash, redirect, g, url_for, jsonify
from flask_login import LoginManager, login_user, logout_user, current_user, login_required
from flask_sqlalchemy import SQLAlchemy
import waitress

__author__ = "Stepan Belousov"
...
class User(db.Model):
    ...
    def __init__(self, username, password):
        ...
        self.password = hashlib.sha512(password.encode('utf8')).hexdigest()
...
def validate_login(form):
    if not re.match(r'\w{4,25}', form['username']):
        return error_msg('username')
    if not re.match(r'(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,25}', form['password']):
...
def validate_register(form):
    ...
    if login_res != 'OK':
        return login_res
    if not re.match(r'(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,25}', form['confirm_password']):
...
def validate_post(form):
    if not re.match(r'\w[\w ]{0,38}\w|\w', form['title']):
        return error_msg('title')
    if not re.match(r'\w[\w ]*\w|\w|^$', form['tags']):
